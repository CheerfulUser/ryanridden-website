...
function animateValue(el, end, duration) {
    ...
    const prefix = end.match(/^[^0-9]*/)[0];   // e.g. "$"
    const suffix = end.match(/[^0-9,\.]*$/)[0]; // e.g. "M" or "+"
    const digits = end.replace(/[^0-9\.]/g, '');
    const numEnd = parseFloat(digits);

    // If the value isn't cleanly numeric (e.g. "$1.4M"), skip animation
    if (isNaN(numEnd) || suffix.length > 1 || prefix.length > 1) {
        el.textContent = end;
        return;
    }
    ...
    function update(now) {
        ...
        el.textContent = prefix + formatted + (suffix.replace('+', '')) + (isPlus ? '+' : '');
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.querySelectorAll('.stat-value').forEach(el => {
                animateValue(el, el.textContent, 1400);
            });
...
document.querySelectorAll('.pub-card, .sw-card, .outreach-section, .contact-card').forEach(el => {
    el.style.opacity = '0';
